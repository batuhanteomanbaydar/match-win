...
class Level1 : Fragment() {
    ...
    override fun onCreateView(
        ...
        var min = 3
        var second = 0
        view.timerText.text = "Time: 0".plus(min.toString()).plus(".00")
        gametimer = object: CountDownTimer(180000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                if(second == 60){
                    view.timerText.text = "Time: 0".plus((min + 1).toString()).plus(":00")
                }else if(second < 10){
                    view.timerText.text = "Time: 0".plus(min.toString()).plus(":0").plus(second.toString())
                }else{
                    view.timerText.text = "Time: 0".plus(min.toString()).plus(":").plus(second.toString())
                }
            }
    ...
    private fun showCards(card: ArrayList<Int>){
        ...
        recyclerView.layoutManager = GridLayoutManager(activity, 3, GridLayoutManager.VERTICAL, true)
                                            ...
                                            if (snapshot.child("nickname").value == user.email!!.split("@")[0]){
                                                ...
                                                database.child(userId!!).child("score").setValue(userScore).addOnCompleteListener {
                                                    database.child(userId!!).child("highscore").setValue(userScore).addOnCompleteListener {
                                                        activity!!.supportFragmentManager.beginTransaction()
                                                            .replace(R.id.root_layout,  Level2())
                                                            .commit()
                                                    }
                                                }
                                                break
